...
@section('extra-lib')
    ...
    <script type="text/javascript">
        ...
        function getFormInfo(id) {
           ...
           // console.log(id);
            $.ajax({
                url: '{!! route("admin.categories.getForm") !!}?id='+id,
                type: 'GET',
            })
            .done(function(output) {
                if(typeof(output.data) == 'undefined'){
                    ...
                    $('#name_category').val(output.data.name_category);
                    $('.category').val(output.data.parent_id);
                }
            })          
        }
        ...
        // console.log(id);
            $('#confirm').modal().one('click', '#delete', function(e){
                $.ajax({
                    url: '{!! route("admin.categories.getDel") !!}',
                    type: 'GET',
                    data: {id : id},
		                    ...
		                    <tr id="tr_{!! $value->id !!}">
		                    	...
		                    	<td>
                                    @if($value->parent_id != -1)
                                        {!! $getMenu[$key] !!}
                                    @else
                                        {!! Form::select('cate', ['' => 'Chưa phân cấp'], null, ['class' => 'form-control']) !!}
                                    @endif
		                    	</td>
    ...
    <div class="modal fade" id="getForm" tabindex="-1" role="dialog" aria-labelledby="PostFormLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                {!!Form::open(['url' => URL::route('admin.categories.postForm'), 'method' => 'POST', 'id' => 'postForm', 'enctype'=>'multipart/form-data'])!!}
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        {!!Form::text('name_category', null, ['class' => 'form-control', 'id' => 'name_category', 'placeholder' => 'Tên danh mục'])!!}
                    </div>
                    <div class="form-group">
                        <label for="parent_id">Phân cấp</label>
                        {!! $menuForm !!}
                    </div>
                </div>
