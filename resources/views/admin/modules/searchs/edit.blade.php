...
@section('content')
    {!! Form::open(['url' => route('admin.searchs.postEdit', ['id' => $search->id])]) !!}
        ...
        <div class="box-body">
            <div class="col-sm-9">
                <div class="form-group">
                    <label>Tiêu đề : </label>
                    <input type="text" name="name_search" class="form-control"  required="required" value="{!! $search->name_search !!}">
                </div>
                <div class="form-group">
                    <label>Danh mục : </label>
                    {!! Form::select('category_id', $categories, $search->category_id, ['class' => 'form-control']) !!}
                </div>
            </div>
            <div class="col-sm-4 _price">
                <p class="title">Thiết lập giá <span id="_price"><i class="fa text-danger fa-remove"></i></span></p>
                <div class="append">
                    @foreach($search->price_searchs as $value)
                        <label>
                            Giá từ <input type="number" name="price[1][]" class="form-control"  required="required" value="{!! $value->price_from !!}"> Đến <input type="number" name="price[1][]" class="form-control" required="required" value="{!! $value->price_to !!}">
                        </label>
                    @endforeach
                </div>
            ...
            <div class="col-sm-4 _color">
                ...
                @if(count($search->colors))
                    @foreach($search->colors as $key => $value)
                        @if($key == 0)
                            <div class="selectColor">
                                {!! Form::select('color[]', $colors, $value->id, ['class' => 'form-control color', 'required' => 'required']) !!}
                            </div>
                            <p class="appendColor">
                        @else
                            {!! Form::select('color[]', $colors, $value->id, ['class' => 'form-control color', 'required' => 'required']) !!}
            ...
            <div class="col-sm-4 _size">
                ...
                @if(count($search->sizes))
                    @foreach($search->sizes as $key => $value)
                        @if($key == 0)
                            <div class="selectSize">
                                {!! Form::select('size[]', $sizes, $value->id, ['class' => 'form-control size', 'required' => 'required']) !!}
                            </div>
                            <p class="appendSize">
                        @else
                            {!! Form::select('size[]', $sizes, $value->id, ['class' => 'form-control size', 'required' => 'required']) !!}
